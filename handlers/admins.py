...
@Client.on_message(command(["control", f"control@{BOT_USERNAME}", "c"]))
@errors
...
async def cbpause(_, query: CallbackQuery):
    if callsmusic.pause(query.message.chat.id):
        ...
        await query.edit_message_text("💤 Nothing is playing, Lol!", reply_markup=BACK_BUTTON)
...
async def cbresume(_, query: CallbackQuery):
    if callsmusic.resume(query.message.chat.id):
        ...
        await query.edit_message_text("💤 Nothing is paused, Lol!", reply_markup=BACK_BUTTON)
...
async def cbmute(_, query: CallbackQuery):
    ...
    if result == 0:
        ...
        await query.edit_message_text("💤 Not in voice chat", reply_markup=BACK_BUTTON)
