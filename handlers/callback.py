...
I'm The Call Music Bot! A Powerful Bot to Play Music in Your Group Voice Chat 😇!
...
Made with ❤️ **@{UPDATES_CHANNEL}**""",
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton(
                        "📦 Source Code 📦", url="https://github.com/brut-ctrl/CallsmusicPlus69"
                    )
                ],
                [
                    InlineKeyboardButton(
...
**Using Player Commands:**
    1. **Group Admin Only Commands 👮 ,**
     - `/play /yp` - Reply to supported url, Reply to Audio File or Send `/play` with [Supported Url](https://ytdl-org.github.io/youtube-dl/supportedsites.html)
       **Example:** `/play https://www.youtube.com/watch?v=ALZHF5UqnU4`
     - `/nplay /pl` - Play Song by Name. (Currenty Supported for Youtube Only)
       **Example:** `/nplay faded`
     - `/skip /s` - Skip currenly playing song.
     - `/pause /p` - Pause currently playing song.
     - `/resume /r` - Resume currently pushed song.
     - `/mute /m` - Mutes Streamer.
     - `/unmute /um`- Unmutes streamer.
     - `/end /e` - Stop playing and leaves the voice chat.
